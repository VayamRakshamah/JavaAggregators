...
public class Java8ParallelAggregator implements Aggregator {
    ...
    public List<Pair<String, Long>> getMostFrequentWords(List<String> words, long limit) {
        Map<String,Long> collectmap = words.parallelStream().collect(Collectors.groupingBy(Function.identity(),Collectors.counting()));
    ...
    public List<String> getDuplicates(List<String> words, long limit) {
        ...
        List<String> final_list = words.parallelStream().map(String::toUpperCase).filter( n -> !duplicates.add(n)).collect(Collectors.toList());
